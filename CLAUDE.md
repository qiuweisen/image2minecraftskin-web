...
## ChartMini v2 SEO/GEO Rules

For SEO/GEO/blog/indexing work in this repository, `docs/seo/v2/_workflow/` is the only active workflow source of truth. Read `docs/seo/README.md` and `docs/seo/v2/_workflow/README.md` before starting an SEO task.

The migrated `docs/seo/flowtrace/_workflow/`, historical `docs/seo/gsc-*` / `docs/seo/bing-*` files, and `/Users/iven/Desktop/Work/Chartmini/TradeGame-cloudflare-migration-test` are read-only history. Do not inherit their GSC/Bing metrics, SERP conclusions, protection windows, submission state, task state, or intent owners without fresh v2 verification.

The v2 blog architecture differs from the old project: source is `content/blog/*.md`, metadata is generated by `scripts/build-blog-manifest.mjs`, article bodies are served through R2, and `src/routes/blog/$slug.tsx` generates the primary BlogPosting/BreadcrumbList/author structured data. Do not assume old article/schema validation rules still apply.

Before editing an SEO target, complete the v2 Preflight and Owner Gate. Do not estimate missing GSC/Bing values. Do not deploy, push, sync R2, or submit indexing unless the user explicitly requests it.
